...
- **Jak przywrócić Google Books, gdyby kiedyś zaczęło zwracać polski katalog:** usunięcie poszło osobnym commitem (`032d6e9` na `develop`, PR #66), rozdzielonym od paginacji **właśnie po to**, żeby dało się je cofnąć samo. `git revert 032d6e9` przywraca całość: `GoogleBooksService`, parametry `source`/`lang`, checkbox, wpisy w `application.yaml` i `docker-compose.yml`, nie ruszając paginacji. Gdy hash się zgubi: `git log --oneline --diff-filter=D -- backend/src/main/java/pl/jaro/restapiworkshop/service/GoogleBooksService.java`. Sam plik bez reszty zmian: `git checkout <hash>^ -- <ścieżka>`, a podejrzenie bez przywracania: `git show <hash>^:<ścieżka>`. W praktyce po dłuższym czasie przydatna będzie tylko treść `GoogleBooksService` (budowa zapytania i mapowanie JSON-a), bo `BookController` zdąży się zmienić
